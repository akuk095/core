// Manage Routines Page JavaScript

// Render routines grouped by type
function renderRoutinesList() {
    const container = document.getElementById('routinesList');
    if (!container) return;

    const allRoutines = loadRoutines();

    if (allRoutines.length === 0) {
        container.innerHTML = `
            <div class="empty-state">
                <div class="empty-state-icon">📋</div>
                <div class="empty-state-text">No routines yet. Add your first routine!</div>
            </div>
        `;
        return;
    }
    ...
    // Define type labels
    const typeLabels = {
        daily: { title: 'Daily Routines', icon: '☀️' },
        weekly: { title: 'Weekly Routines', icon: '📅' },
        fortnightly: { title: 'Fortnightly Routines', icon: '📆' },
        monthly: { title: 'Monthly Routines', icon: '🗓️' }
    };

    // Render each type group
    Object.entries(routinesByType).forEach(([type, routines]) => {
        if (routines.length > 0) {
            html += `<h3 class="section-title">${typeLabels[type].icon} ${typeLabels[type].title}</h3>`;

            routines.forEach(routine => {
                const totalDuration = routine.tasks.reduce((sum, task) => sum + task.duration, 0);
                const scheduleText = getScheduleText(routine);

                html += `
                    <div class="routine-list-card" onclick="editRoutine('${routine.id}')">
...
// Get schedule text for a routine
function getScheduleText(routine) {
    const dayNames = ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'];

    if (routine.type === 'daily') {
        return `${routine.startTime} • Every day`;
    } else if (routine.type === 'weekly' && routine.schedule && routine.schedule.weekDays) {
        const days = routine.schedule.weekDays.map(d => dayNames[d]).join(', ');
        return `${routine.startTime} • ${days}`;
    } else if (routine.type === 'fortnightly' && routine.schedule) {
        return `${routine.startTime} • Every 2 weeks on ${dayNames[routine.schedule.weekDay]}`;
    } else if (routine.type === 'monthly' && routine.schedule) {
        return `${routine.startTime} • Day ${routine.schedule.dayOfMonth} of month`;
    }
...
// Edit routine (placeholder - can be expanded later)
function editRoutine(routineId) {
    alert('Edit functionality coming soon! Routine ID: ' + routineId);
